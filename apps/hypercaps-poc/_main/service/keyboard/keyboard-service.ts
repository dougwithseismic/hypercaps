...
export class KeyboardService extends EventEmitter {
  ...
  private handleConfigChange(config: RemapperConfig): void {
    ...
    if (!this.validateConfig(config)) {
      console.error(
        '[KeyboardService] Invalid remap configuration, not applying changes'
      );
      return;
    }
  ...
  public async startListening(): Promise<void> {
    ...
    try {
      const config: KeyboardConfig = {
        ...
        capsLockBehavior:
          remapper.config.capsLockBehavior || DEFAULT_CAPS_BEHAVIOR,
  ...
  public dispose(): void {
    ...
    this.unsubscribeHandlers.forEach((unsubscribe) => {
      try {
        ...
        console.error(
          '[KeyboardService] Error during listener cleanup:',
          error
        );
      }
    });
