...
export class StoreManager {
  ...
  public async migrate(targetVersion: string) {
    const result = await this.migrationManager.migrate(this.store.store, targetVersion)
   ...
   * ```
   */
  public setFeatureEnabled<K extends keyof Features>(feature: K, enabled: boolean): void {
    const features = this.store.get('features')
    const current = features[feature]
    const newFeatures = {
      ...features,
