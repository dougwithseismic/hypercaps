...
const keyboardMonitor = require("@hypercaps/keyboard-monitor");
...
const HyperKeyFeatureConfigSchema = z.object({
...
const _Store = class _Store extends events.EventEmitter {
    ...
    for (const migration of sortedMigrations) {
      if (this.compareVersions(version, migration.version) < 0) {
        ...
        state = this.validateState(
          state,
          (nextMigration == null ? void 0 : nextMigration.schema) || AppStateSchema
        );
      }
    }
  ...
  async load() {
    try {
      if (fs.existsSync(this.statePath)) {
        ...
        const versionedState = JSON.parse(data);
        this.state = this.migrateState(versionedState);
        if (!versionedState.version || this.compareVersions(versionedState.version, CURRENT_STATE_VERSION) < 0) {
          await this.save();
        }
      } else {
  ...
  async getFeature(name) {
    var _a;
    const feature = (_a = this.state.features) == null ? void 0 : _a.find((f) => f.name === name);
    return feature;
  }
};
...
const _IPCService = class _IPCService {
  ...
  registerHandler(serviceId, action, handler) {
    ...
    if (!this.eventHandlers.has(serviceId)) {
      this.eventHandlers.set(serviceId, /* @__PURE__ */ new Map());
    }
    const serviceHandlers = this.eventHandlers.get(serviceId);
    if (!serviceHandlers.has(action)) {
      serviceHandlers.set(action, /* @__PURE__ */ new Set());
    }
...
class KeyboardService extends events.EventEmitter {
  constructor() {
    ...
    __publicField(this, "keyboardMonitor", null);
    ...
    __publicField(this, "handleKeyboardFrame", (data) => {
      var _a;
      const keyboardState = {
        frame: data.frame,
        ...
        event: {
          type: "keydown",
          key: String(data.state.justPressed[0] || "")
          // Use first pressed key if available
        },
        state: {
  ...
  async startListening() {
    ...
    try {
      ...
      this.keyboardMonitor = new keyboardMonitor.KeyboardMonitor(
        (eventName, data) => {
          if (eventName === "frame") {
...
class ShortcutService {
  ...
  async toggleEnabled() {
    await this.store.update((draft) => {
      const feature = draft.features.find((f) => f.name === "shortcutManager");
      if (feature) {
        feature.config.isEnabled = !feature.config.isEnabled;
      }
    });
  }
}
  ...
  async updateContextMenu() {
    ...
    const contextMenu = electron.Menu.buildFromTemplate([
      ...
      { type: "separator" },
      {
        ...
        type: "checkbox",
        checked: (hyperKey == null ? void 0 : hyperKey.config.isHyperKeyEnabled) || false,
      ...
      },
      {
        ...
        type: "checkbox",
        checked: (hyperKey == null ? void 0 : hyperKey.enableFeatureOnStartup) || false,
        click: async (menuItem) => {
          await this.store.update((draft) => {
            const feature = draft.features.find((f) => f.name === "hyperKey");
            if (feature) {
      ...
      },
      {
        ...
        type: "checkbox",
        checked: (hyperKey == null ? void 0 : hyperKey.config.capsLockBehavior) === "BlockToggle",
        click: async (menuItem) => {
          var _a2;
          await this.store.update((draft) => {
            const feature = draft.features.find((f) => f.name === "hyperKey");
        ...
        submenu: [
          {
            ...
            type: "checkbox",
            checked: (settings == null ? void 0 : settings.startupOnBoot) || false,
            click: async (menuItem) => {
              await this.store.update((draft) => {
          ...
          },
          {
            ...
            type: "checkbox",
            checked: (settings == null ? void 0 : settings.startMinimized) || false,
            click: async (menuItem) => {
              await this.store.update((draft) => {
                if (!draft.settings) draft.settings = {};
...
electron.app.whenReady().then(async () => {
  ...
  electron.ipcMain.handle("get-startup-settings", async () => {
    ...
    return {
      ...
      startMinimized: ((_b = state.settings) == null ? void 0 : _b.startMinimized) || false
    };
  });
